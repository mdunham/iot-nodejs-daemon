...
for (var ii = 0; ii < msg.length; ii++) {
	if ('string' === typeof msg[ii]) {
		byteArr.push(msg[ii].charCodeAt(0));
		ustr = ustr + msg[ii]
	} else {
		byteArr.push(msg[ii]);
		ustr = ustr + String.fromCharCode(msg[ii]);
	}
}

data = byteToHexString(Encodeuint8arr(ustr));
data.map((byte) => {
	var XORFlag;
	for (var i = 7; i >= 0; --i) {
...
data.push((crc % 0x0100));
data.push((crc / 0x0100));


function prepareMessage() {

// ~~<to><from><status><len the number of data bytes><data0>...<datalen-1><crc0><crc1>
	...
	data = byteToHexString(byteArr);

	console.log(data, crc, byteToHexString([((crc >> 8) & 0xFF), (crc & 0xFF)]));
