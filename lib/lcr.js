...
const logLcr  = function(log) {
	let 
		m = new Date(), 
		time = m.getUTCFullYear() + "/" +
			("0" + (m.getUTCMonth()+1)).slice(-2) + "/" +
			("0" + m.getUTCDate()).slice(-2) + " " +
			("0" + m.getUTCHours()).slice(-2) + ":" +
			("0" + m.getUTCMinutes()).slice(-2) + ":" +
			("0" + m.getUTCSeconds()).slice(-2) + ' - ';
	lcrLog.write(time + log + '\n');
};
...
console.error = console.log = function() {
	let 
		...
		time = m.getUTCFullYear() + "/" +
			...
			("0" + m.getUTCMinutes()).slice(-2) + ":" +
			("0" + m.getUTCSeconds()).slice(-2) + ' - ';
	stdFile.write(time + util.format.apply(null, arguments) + '\n');
	process.stdout.write(time + util.format.apply(null, arguments) + '\n');
};
...
module.exports.device = function(deviceTTY = 'ttyUSB0', node = 250, port = 255) {
	...
	self.init = function(reconnect) {
		var
			...
			sync: (count) => {
				...
				self.checkStatus(function(status, product, version) {
					if ( ! status) {
						...
						return setTimeout(() => steps.sync(++count), 250);
					}
			...
			getDecimals: (count) => {
				...
				self.getDecimals(function(precision) {
					if ( ! precision) {
						...
						return setTimeout(() => steps.getDecimals(++count), 250);
					}
			...
			getTotalNet: (count) => {
				...
				self.getTotalNet((volume) => {
					if (false === volume) {
						...
						return setTimeout(() => steps.getTotalNet(++count), 250);
					}
			...
			getTotalGross: (count) => {
				...
				self.getTotalGross((volume) => {
					if (false === volume) {
						...
						return setTimeout(() => steps.getTotalGross(++count), 250);
					}
			...
			getShiftCount: (count) => {
				...
				self.getShiftCount((deliveries) => {
					if (false === deliveries) {
						...
						return setTimeout(() => steps.getShiftCount(++count), 250);
					}
			...
			getShiftNet: (count) => {
				...
				self.getShiftNet((volume) => {
					if (false === volume) {
						...
						return setTimeout(() => steps.getShiftNet(++count), 250);
					}
			...
			getShiftGross: (count) => {
				...
				self.getShiftGross((volume) => {
					if (false === volume) {
						...
						return setTimeout(() => steps.getShiftGross(++count), 250);
					}
			...
			checkPrinter: (count) => {
				...
				self.checkPrinter((ticketRequired) => {
					...
					if (ticketRequired && count < 10) {
						self.disablePrinter((status, deviceByte) => {
							setTimeout(() => steps.checkPrinter(++count), 250);
						});
					} else {
						if ( ! ticketRequired && count < 10) {
				...
				clearTimeout(retry);
				logLcr('Initialized - Total Gross: ' + lcrTotals.totalGross + ' Total Net: ' + 
	...
	self.getWithDecimal = function(value, decimals) {
		...
		return value.substring(0, value.length - decimals) + '.' + value.substring(value.length - decimals);
	};
...
//		getFields(0);
	...
	self.checkPrinter = function(callback) {
		var _call = callback;
		self.getField(0x25, (status, deviceByte, data) => _call(data === '00'));
	};
