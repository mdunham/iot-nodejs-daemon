	...
	self.pulseStatus = function() {
		var
			...
			getFields = function(index) {
				...
				var wId = setTimeout(function(){ getFields(++index); }, 300);
				self.getField(_getFields[index], function(status, deviceByte, data) {
					...
					if (data && data.length) {
						if (index === 0) {
							...
							_fieldData[_fieldDesc[index]] = view.getInt32(0);
							console.log('Dec: ', _decimals);
							if (_decimals) _fieldData[_fieldDesc[index]] = _fieldData[_fieldDesc[index]].substr(0, _fieldData[_fieldDesc[index]].length - _decimals) + '.' + _fieldData[_fieldDesc[index]].substr(_fieldData[_fieldDesc[index]].length - _decimals);
						}
						if (status) {
		...
		getFields(0);
		setTimeout(self.pulseStatus, 2900);
	},
