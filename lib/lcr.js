...
module.exports.device = function(deviceTTY = 'ttyUSB0', node = 250, port = 255) {
	...
	self.pulseStatus = function() {
		setTimeout(function(){
			var
				...
				getFields = function(index) {
					...
					var wId = setTimeout(function(){ getFields(++index); }, 100);
					self.getField(_getFields[index], function(status, deviceByte, data) {
						...
						if (data && data.length) {
							if (index === 0) {
								...
								if (_decimals) _fieldData[_fieldDesc[index]] = _fieldData[_fieldDesc[index]].splice(_fieldData[_fieldDesc[index]].length - (_decimals + 1), 0, '.')
							}
							if (status) {
		...
		setTimeout(self.pulseStatus, 1000);
	},
