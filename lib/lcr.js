...
module.exports.device = function(deviceTTY = 'ttyUSB0', node = 250, port = 255) {
	...
	self.pulseStatus = function() {
		var
			_fieldData = {},
			_fieldDesc = ['vol-decimals', 'gross-qty-current', 'net-qty-current', 'flow-rate', 'gross-preset', 'gross-qty-current-shift', 'prod-com-gross', 'gross-totalizer-previous', 'abs-gross-qty-current', 'net-qty-remaining'],
			_getFields = [0x27, 0x02, 0x03, 0x04, 0x05, 0x0D, 0x11, 0x64, 0x2C, 0x5D], 
			getFields = function(index) {
				if (index == _getFields.length) {
					if (_fieldData['gross-qty-current'] !== '.0' && _fieldData['prod-com-gross'] !== '.0' && _fieldData['gross-qty-current-shift'] !== '.0')
						...
						setTimeout(self.pulseStatus, 200);
					return;
				}
				var wId = setTimeout(function(){ getFields(index+1); }, 200);
