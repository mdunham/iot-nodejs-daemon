...
module.exports.device = function(deviceTTY = 'ttyUSB0', node = 250, port = 255) {
	...
	self.pulseStatus = function() {
				...
				self.getField(_getFields[index], function(status, deviceByte, data) {
					...
					if (data && data.length) {
						if (index === 0) {
							...
							if (_decimals) _fieldData[_fieldDesc[index]] = _fieldData[_fieldDesc[index]].toString().substring(0, _fieldData[_fieldDesc[index]].length - _decimals) + '.' + _fieldData[_fieldDesc[index]].toString().substring(_fieldData[_fieldDesc[index]].length - _decimals);
						}
						if (status) {
		...
		getFields(0);
		setTimeout(self.pulseStatus, 2800);
	},
