...
module.exports.device = function(deviceTTY = 'ttyUSB0', node = 250, port = 255) {
	...
	self.init = function(reconnect) {
		var
			...
			connect: (count) => {
				...
				self.connect((status) => {
					if ( ! status) {
						console.log('LCR::init -> No LCR Detected After ' + count + ' attempts.');
						clearTimeout(retry);
						return setTimeout(() => steps.connect(++count), 150);
					}
					clearTimeout(retry);
			...
			sync: (count) => {
				...
				self.checkStatus(function(status, product, version) {
					if ( ! status) {
						...
						clearTimeout(retry);
						return setTimeout(() => steps.sync(++count), 150);
					}
					
					self.lcrVersion = version;
					self.lcrProduct = product;
					clearTimeout(retry);
					setTimeout(() => steps.clear(0), 100);
				});
			},
			
			// Verify that we dont have a pending transaction
			clear: (count) => {
				retry = setTimeout(() => steps.clear(++count), 2000);
				self.removeTransaction(function(status, product, version) {
					if ( ! status) {
						clearTimeout(retry);
						return setTimeout(() => steps.sync(++count), 150);
					}
			...
			status: (count) => {
				...
				self.getDeliveryStatus(function(status, data) {
					...
					self.lcrStatus = data;
					clearTimeout(retry);
					setTimeout(() => steps.getDecimals(0), 100);
				});
			},
			...
			getDecimals: (count) => {
				...
				self.getDecimals(function(precision) {
					if ( ! precision) {
						...
						clearTimeout(retry);
						return setTimeout(() => steps.getDecimals(++count), 150);
					}
					
					clearTimeout(retry);
					setTimeout(() => steps.getTotalNet(0), 100);
				});
			},
			...
			getTotalNet: (count) => {
				...
				self.getTotalNet((volume) => {
					if (false === volume) {
						...
						clearTimeout(retry);
						return setTimeout(() => steps.getTotalNet(++count), 150);
					}
					
					lcrTotals.totalNet = volume;
					clearTimeout(retry);
					setTimeout(() => steps.getTotalGross(0), 100);
				});
			},
			...
			getTotalGross: (count) => {
				...
				self.getTotalGross((volume) => {
					if (false === volume) {
						...
						clearTimeout(retry);
						return setTimeout(() => steps.getTotalGross(++count), 150);
					}
					
					lcrTotals.totalGross = volume;
					clearTimeout(retry);
					setTimeout(() => steps.getShiftCount(0), 100);
				});
			},
			...
			getShiftCount: (count) => {
				...
				self.getShiftCount((deliveries) => {
					if (false === deliveries) {
						...
						clearTimeout(retry);
						return setTimeout(() => steps.getShiftCount(++count), 150);
					}
					
					lcrTotals.shiftDeliveries = deliveries;
					clearTimeout(retry);
					setTimeout(() => steps.getShiftNet(0), 100);
				});
			},
			...
			getShiftNet: (count) => {
				...
				self.getShiftNet((volume) => {
					if (false === volume) {
						...
						clearTimeout(retry);
						return setTimeout(() => steps.getShiftNet(++count), 150);
					}
					
					lcrTotals.totalNet = volume;
					clearTimeout(retry);
					setTimeout(() => steps.getShiftGross(0), 100);
				});
			},
			...
			getShiftGross: (count) => {
				...
				self.getShiftGross((volume) => {
					if (false === volume) {
						...
						clearTimeout(retry);
						return setTimeout(() => steps.getShiftGross(++count), 150);
					}
					
					lcrTotals.shiftGross = volume;
					clearTimeout(retry);
					setTimeout(() => steps.checkPrinter(0), 100);
				});
			},
			...
			checkPrinter: (count) => {
				...
				self.checkPrinter((ticketRequired) => {
					...
					if (ticketRequired && count <= 10) {
						console.log('disable printer');
						self.disablePrinter((status, deviceByte) => {
							clearTimeout(retry);
							setTimeout(() => steps.checkPrinter(++count), 150);
						});
					} else {
						if ( ! ticketRequired || count >= 10) {
	...
	self.disablePrinter = function(callback) {
		var _call = callback;
		device.write(Buffer.from('7E7EFAFF00032125019C05', 'hex'), _call);
		//device.write(Buffer.from('7E7EFAFF0103212502AF32', 'hex'), _call);
	};
	...
	self.getShiftNet = function(callback) {
		...
		self.getField(0x0E, (status, deviceByte, data) => {
			...
			console.log('getShiftNet -> ' + ascii.toVolume(data, _decimals), data);
	...
	self.getShiftGross = function(callback) {
		...
		self.getField(0x0D, (status, deviceByte, data) => {
			...
			console.log('Shift Gross -> ' + ascii.toVolume(data, _decimals), data);
			_call(ascii.toVolume(data, _decimals));
		});
	};
	
	/**
	 * Get the current deliveries remaining gross preset
	 * 
	 * @param {Function} callback
	 * @returns {void}  
	 */
	self.presetStatus = function(callback) {
		var  _call = callback;
		self.getField(0x5C, (status, deviceByte, data) => {
			if ( ! status) {
				return _call(false);
			}
			
			console.log('Gross Remaining -> ' + ascii.toVolume(data, _decimals), data);
			_call(ascii.toVolume(data, _decimals));
		});
	};
	
	/**
	 * Get the current deliveries gross
	 * 
	 * @param {Function} callback
	 * @returns {void}  
	 */
	self.getDeliveryGross = function(callback) {
		var  _call = callback;
		self.getField(0x2C, (status, deviceByte, data) => {
			if ( ! status) {
				return _call(false);
			}
	...
	self.getTotalNet = function(callback) {
		...
		self.getField(0x65, (status, deviceByte, data) => {
			...
			console.log('getTotalNet -> ' + ascii.toVolume(data, _decimals), data);
			_call(ascii.toVolume(data, _decimals));
		});
	};
	...
	self.getTotalGross = function(callback) {
		...
		self.getField(0x64, (status, deviceByte, data) => {
			...
			console.log('getTotalGross -> ' + ascii.toVolume(data, _decimals), data);
			_call(ascii.toVolume(data, _decimals));
		});
	};
	...
	self.getField = function(fieldNum, callback) {
		...
		return lcRequest.issue([lcFlags.lcrMethods.GET_FIELD, fieldNum], (status, response) => {
			if ( ! status) {
				console.info('Error getField: ' + response);
				return false;
			}
			...
			var 
				...
				fieldValue  = data.substring(4);
			
			console.info('LCR Get Field - ' + fieldNum + ' Data: ' + data, response.buffer, fieldValue.match(/../g));
	...
	self.setField = function(fieldNum, rawData, callback) {
		...
		return lcRequest.issue(data, (status, response) => {
			...
			console.log('LCR Set Field - ' + fieldNum + ' ret: ' + returnCode + ' Dev: ' + deviceByte);
	...
	self.getDeliveryStatus = function(callback) {
		...
		return lcRequest.issue([lcFlags.lcrMethods.DELIVERY_STATUS], (status, response) => {
			...
			var 
				data          = response.data.toString(),
				returnCode    = data.substr(0,2),
				deviceStat    = lcFlags.machine.getCode(parseInt(data.substr(2,2), 16)),
				deliveryStat  = lcFlags.delivery.getStatus(parseInt(data.substr(4,4), 16)),
				deliveryCode  = lcFlags.delivery.getCode(parseInt(data.substr(8,4), 16));
				
			console.info('LCR Machine Status - ' + parseInt(data.substr(2,2), 16), deviceStat, deliveryStat, deliveryCode);
			_call(status, {
				returnCode: returnCode,
