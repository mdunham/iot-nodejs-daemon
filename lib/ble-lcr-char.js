...
class LcrCharacteristic extends bleno.Characteristic {
	...
	onWriteRequest(data, offset, withoutResponse, callback) {
		var self = this, cmdArgs = data.toString(), flowReadout;
...
//					break;
				case 'sync':
					...
					break;
				case 'pause_delivery':
					this._device.command(0x01, (status, returnCode, deviceStatus) => {
						if ( ! status) {
					...
					break;
				case 'stop_delivery':
					this._device.command(0x02, (status, returnCode, deviceStatus) => {
						if ( ! status) {
							console.info('Command Failed');
							this.updateValueCallback(new Buffer('fail'));
						} else {
				...
				case 'resume_delivery':
					this._device.command(0x00, (status, returnCode, deviceStatus) => {
						if ( ! status) {
							console.info('Command Failed');
							this.updateValueCallback(new Buffer('fail'));
						} else {
...
//						console.log('Preset: ' + returnCode);
						this._device.command(0x00, (status, returnCode, deviceStatus) => {
							if ( ! status) {
								...
								this.updateValueCallback(new Buffer('ok|' + returnCode + '|' + deviceStatus));
								clearInterval(flowReadout);
								flowReadout = setInterval(() => {
									this._device.getDeliveryGross((grossRemaining) => {
										if ('function' !== typeof this.updateValueCallback)
											return clearInterval(flowReadout);
										this.updateValueCallback(new Buffer('preset|' + grossRemaining));
									});
								}, 1500);
							}
						});
