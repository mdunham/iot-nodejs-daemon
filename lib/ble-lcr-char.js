...
class LcrCharacteristic extends bleno.Characteristic {
	constructor() {
		...
		require('fs').readFile('/etc/cl-lcr-truck', (err, data) => {
			...
			//else self.truck_id = data.replace(/\n/g, '');
		});
	}
				...
				case 'new_delivery':
					this._device.activatePumpPrint((status, returnCode) => {
						if (status) {
							...
							view.setUint32(0, parseInt(cmdArgs[1]), false);
							this._device.setField(5, arr, (status, returnCode) => {});
					...
					break;
				case 'get_totalizer':
					this._device.getField(0x5E, (status, deviceByte, data) => {
						let
							totalizer,
							buf = new ArrayBuffer(4),
							view = new DataView(buf);

						data.match(/../g).forEach(function (b, i) {
